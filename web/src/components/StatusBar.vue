...
<template>
  <div class="flex items-center gap-2 text-xs">
    <!-- Cluster health -->
    <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-gray-800/50 font-medium" :class="clusterColor">
    ...
    <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-gray-800/50 text-gray-300 font-medium">
      ...
      <span :class="deviceCount.online > 0 ? 'text-emerald-400' : ''">{{ deviceCount.online }}</span>/<span>{{ deviceCount.total }}</span>
    </span>
    ...
    <!-- Credits -->
    <span v-if="creditBalance !== null" class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-gray-800/50 text-gray-300 font-medium">
      <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
      {{ creditBalance }}
    </span>

    <!-- UTC clock -->
    <span class="font-mono text-gray-400 px-1">{{ utcTime }}</span>
  </div>
</template>
