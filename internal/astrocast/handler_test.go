...
func newTestPayload(deviceGUID, msgGUID, text string) MOPayload {
	return MOPayload{
		DeviceGUID:   deviceGUID,
		MessageGUID:  msgGUID,
		Data:         base64.StdEncoding.EncodeToString([]byte(text)),
		TransmitTime: "2026-03-19T10:00:00Z",
		Latitude:     52.1621,
		Longitude:    4.5094,
	}
}
...
func TestServeHTTP_ValidPayload(t *testing.T) {
	...
	if mb.published[0].topic != "meshsat/dev-001/mo/raw" {
		t.Errorf("expected mo/raw topic, got %s", mb.published[0].topic)
	}
	if mb.published[1].topic != "meshsat/dev-001/mo/decoded" {
...
func TestServeHTTP_ValidSignature(t *testing.T) {
	...
	payload := newTestPayload("dev-001", "msg-abc", "Hello signed")
...
func TestServeHTTP_PositionPublished(t *testing.T) {
	...
	if mb.published[2].topic != "meshsat/dev-001/position" {
...
func TestServeHTTP_DecodedMessageChannel(t *testing.T) {
	...
	// Check the decoded message has channel "astrocast".
	var decoded MOMessage
	_ = json.Unmarshal(mb.published[1].data, &decoded)
	if decoded.Channel != "astrocast" {
		t.Errorf("expected channel astrocast, got %s", decoded.Channel)
	}
	if decoded.DeviceGUID != "dev-001" {
		t.Errorf("expected device_guid dev-001, got %s", decoded.DeviceGUID)
	}
}
